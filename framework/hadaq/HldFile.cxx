...
{
   if (!isReading() || !ptr || !sz || (*sz < sizeof(hadaqs::HadTu))) return false;
   ...
   if (onlyevent) {
      ...
      if ( readsz != (hdr->GetPaddedSize() - sizeof(hadaqs::HadTu))) {
         fprintf(stderr, "HLD file reading problem: readsz %lu not matches with padded size %lu, abort reading\n", (long unsigned) readsz, (long unsigned) (hdr->GetPaddedSize() - sizeof(hadaqs::HadTu)));
         fEOF = true;
         return false;
      }
