...
namespace hadaq {
   ...
   class TdcIterator {
      ...
      public:
         ...
         inline double get400MsgTimeCoarse(unsigned coarse25, double mhz) const
         { return fConv.ToSeconds((isCurEpoch() ? ((uint64_t) fCurEpoch) << 11 : 0) | (coarse25 >> 1)) + (coarse25 % 2) * 1e-9 * (1000 / mhz); }
