...
namespace hadaq {
   ...
   struct MessageCompact {
      uint8_t ch;    // channel and edge
      float   stamp; // time stamp minus channel0 time
      ...
      MessageCompact() : ch(0), stamp(0.) {}
      MessageCompact(const MessageCompact& src) : ch(src.ch), stamp(src.stamp) {}
      MessageCompact(unsigned _ch, bool _rising, float _stamp) :
